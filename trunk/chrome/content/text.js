...
/**
 * Remove slashes before single and double quote
 *
 * @author Fragile
 */
...
function parsePhp( fileContent, type )
{
	if( type == 'source' ) 
		...
		else if( thisChar == openString )
		{
			...
			if( lastVar != '' && isDefined(lastVar) )
			{
				if( typeIsSource )
				{
					...
					fields[lastVar]['original'] = stripslashes(fileContent.substr( lastOpenPos, i - lastOpenPos )); //string to translate						
				}
				else
				{
					if( typeof(fields[lastVar]) != 'undefined' )
					{
						fields[lastVar]['translation'] = stripslashes(fileContent.substr( lastOpenPos, i - lastOpenPos ));
					}
				}
			}
			openString = '';
...
function parseIni( fileContent, type )
{
	if( type == 'source' ) 
		var typeIsSource = true;
	else
	...
	for( var i = 0; i < lines.length; i++ )
	{
		thisLine = lines[i];
		// skip empty lines 
		if( trim(thisLine) == '' ) continue;
		
		// var key_val = thisLine.split(/\s?=\s?/);
		var key_val = thisLine.match(/^(#+.*|[^=\s]*)(\s?=\s?(.*))?/);
		// lastVar = key_val[0];
		lastVar = key_val[1];
		if( key_val == false || key_val.length < 3 || key_val[2] == '' )
		{
			lastVar = thisLine;
			//this is a comment, an ini section or something similar, don't translate
			fields[lastVar] = new Array();
			fields[lastVar]['original'] = '';
			fields[lastVar]['translation'] = '';
			fields[lastVar]['stringDelimiter'] = '';
		}
		else
		{
			if( typeIsSource )
			{
				...
				{					
					fields[lastVar] = new Array();
					fields[lastVar]['translation'] = ''; // translation
				}
				fields[lastVar]['original'] = key_val[3]; //string to translate						
				fields[lastVar]['stringDelimiter'] = ''; // no need in .ini but respect common structure
			}
			else
			{
				if( typeof(fields[lastVar]) != 'undefined' )
				{
					fields[lastVar]['translation'] = key_val[3];
				}
			}
		}
	}
